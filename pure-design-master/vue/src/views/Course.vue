...
export default {
  ...
  methods: {
    selectCourse(courseId) {
      this.axios.post('/course/studentCourse/' + courseId + "/" + this.user.id).then(res => {
        if (res.code === '200') {
    ...
    load() {
      ...
      this.axios.get("/user/role/ROLE_TEACHER").then(res => {
        this.teachers = res.data
      })
    },
    changeEnable(row) {
      this.axios.post("/course/update", row).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.axios.post("/course/del/batch", ids).then(res => {
        if (res.code === '200') {
